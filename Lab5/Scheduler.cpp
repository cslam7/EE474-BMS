...
void fillSchedule(TCB* dataPtr, TCB* taskQueue[]) {
    ...
    if (global_counter % 1 == 0) {
        insertNode(taskQueue[1]);
        insertNode(taskQueue[2]);
        insertNode(taskQueue[3]);
        insertNode(taskQueue[4]);
    }
...
//        (*(taskQueue[0])).task((*(taskQueue[0])).taskDataPtr);
//        (*(taskQueue[0])).task((*(taskQueue[6])).taskDataPtr);
    ...
    global_counter++;
//    Serial.println("I ran thru one second of the stuff");
	return;
}
